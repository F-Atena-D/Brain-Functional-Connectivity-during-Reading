...
% Script Name: ConnGraphPlot
% Author: Fatemeh Delavari 
% Version: 1.0
% Description: This script plots the Functional connectivity graphs on scalp EEG
% topographic maps
...
%%
mplvt = squeeze(mean(plvt, 1)); % Average over all subjects

%% Calculate the adjacency matrices of the template Graph and Graphs at 20, 200, and 480 ms
plvA = squeeze(mean(mplvt(1:125, :, :)));
plv20 = squeeze((mplvt(5, :, :)));
plv200 = squeeze((mplvt(50, :, :)));
plv480 = squeeze((mplvt(120, :, :)));

%% plot the Scalp EEG Topographic Plot
...
%% Template Graph
tr = .7; % Apply threshold and plot the unweithed Graph
tplv = plvA;
    tplv(tplv>tr) = 1;
    tplv(tplv<tr) = 0;
    tplv(tplv==tr) = 1;
    tplvA = tplv;
G = graph(tplv, 'upper');
edg = G.Edges.EndNodes;

hold on;
for i=1:length(edg)
plot([y(edg(i,1), 1), y(edg(i,2), 1)], [x(edg(i,1), 1), x(edg(i,2), 1)],'Color',[0 0.4470 0.7410], 'LineWidth', 1.7);
end
...
hold on;
for i=1:length(edg)
plot([y(edg(i,1), 1), y(edg(i,2), 1)], [x(edg(i,1), 1), x(edg(i,2), 1)],'Color',[0.3010 0.7450 0.9330], 'LineWidth', 1.7);
end
